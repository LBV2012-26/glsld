...
namespace glsld {
    TypeResolver::TypeResolver(const DocumentSymbols& symbols)
        : symbols_{ symbols }
    {}
    ...
    void TypeResolver::VisitVariableDeclaration(VariableDeclarationNode* node) {
        if (node->declared_symbol != nullptr) {
            ...
            auto* type_symbol = current_scope_->FindSymbol(node->declared_symbol->type_info.typename_token.text);
            if (type_symbol != nullptr &&
                (type_symbol->kind == SymbolKind::kInterface || type_symbol->kind == SymbolKind::kStruct))
            {
    ...
    void TypeResolver::VisitVariableExpression(VariableExpressionNode* node) {
        if (node->linked_symbols.index() == 0) {
    ...
    void TypeResolver::VisitMemberAccessExpression(MemberAccessExpressionNode* node) {
        ...
        if (struct_symbol != nullptr && struct_symbol->body_scope != nullptr) {
            auto* member_symbol = struct_symbol->body_scope->FindSymbol(node->member->name);
